...
/** JSON de taxonomía: { Depto: { Cat: { Sub: [] } } } */
export type TaxonomyDict = Record<string, Record<string, Record<string, unknown[]>>>;

export const exportTaxonomy = (signal?: AbortSignal) =>
  request<{ taxonomy: TaxonomyDict; company_id: number }>("/taxonomy/export", {
    signal,
  });
...
export const bootstrapTaxonomy = (taxonomy: TaxonomyDict) =>
  request<{
    ...
    stats: {
      ...
      departments_inserted: number;
      categories_inserted: number;
      subcategories_inserted: number;
    };
